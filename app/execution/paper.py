...
class MultiLegPaperOrder:
    ...
    def net_premium(self) -> float:
        ...
        for leg in self.legs:
            side = leg.side.upper()
            if side == "BUY":
                net += float(leg.entry_price) * leg.filled_quantity
            elif side == "SELL":
                net -= float(leg.entry_price) * leg.filled_quantity
        return net
...
class PaperPosition:
    ...
    def apply_trade(self, order: PaperOrder) -> None:
        ...
        if action in {"BUY", "LONG"}:
            if self.quantity >= 0:
                total_cost = self.average_price * max(self.quantity, 0) + order.entry_price * qty
                ...
                if remaining_short > 0:
                    ...
                    self.quantity = qty - close_qty
                    self.average_price = order.entry_price if self.quantity > 0 else 0.0
        elif action in {"SELL", "SHORT"}:
            if self.quantity <= 0:
                short_qty = abs(self.quantity)
                total_cost = self.average_price * max(short_qty, 0) + order.entry_price * qty
                new_qty = self.quantity - qty
                self.quantity = new_qty
...
class PaperExecutionEngine:
    ...
    def submit_multi_leg(self, strategy: str, trade_id: str, legs: List[PaperLeg]) -> MultiLegPaperOrder:
        ...
        multi = MultiLegPaperOrder(strategy=strategy, trade_id=trade_id, legs=legs)
